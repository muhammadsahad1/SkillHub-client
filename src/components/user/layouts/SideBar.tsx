...
const SideBar = ({ onClose, onSelect }: any) => {
  return (
    <div
      className={`bg-zinc-100 shadow-md fixed top-16 left-0 z-40 w-full md:w-1/4 lg:w-1/6 h-screen transition-transform ${
        onClose ? "transform -translate-x-full" : ""
      }`}
    >
      <button
        onClick={onClose}
      ...
      <div className="mt-10">
        <ul className="font-bold font-poppins text-zinc-950 cursor-pointer ms-2">
          <NavLink to="/auth/settings" onClick={() => onSelect("settings")}>
            {({ isActive }) => (
              <li
                className={`border-b border-zinc-100 py-2 hover:bg-zinc-200 ${
